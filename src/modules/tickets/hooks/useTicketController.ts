...
export const useTicketController = () => {
  ...
  const handleEdit = async (id: number, data: TicketRequest) => {
    try {
      ...
      return { success: true, message: "လက်မှတ်အချက်အလက်ကို အောင်မြင်စွာ ပြင်ဆင်ပြီးပါပြီ။" }; // Ticket updated successfully
    } catch (error: any) {
      return {
        success: false,
  ...
  const handleCreate = async (data: any) => {
    try {
      ...
      return { success: true, message: "လက်မှတ်ကို အောင်မြင်စွာ ပြုလုပ်ပြီးပါပြီ။" };  
    } catch (error: any) {
      return {
        success: false,
